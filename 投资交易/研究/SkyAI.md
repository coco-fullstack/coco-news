---
title: SkyAI
date: 2026-02-18
tags:
  - research/token
  - token/SKYAI
related:
  - "[[AI Agent]]"
  - "[[Crypto x AI 全景]]"
status: active
---

## 基本信息

| 项目 | 内容 |
|------|------|
| 全称 | SkyAI |
| 代币 | SKYAI |
| 排名 | #553 |
| 赛道 | BNB Chain Ecosystem, Binance Alpha Spotlight |
| 官网 | https://skyai.pro/ |
| Twitter | https://x.com/SKYAIpro |

## 项目简介

SkyAI 是一个基于 MCP（Model Context Protocol）的一站式 AI 生态系统，旨在跨行业无缝集成智能解决方案。

## 实时行情

> [TradingView K线图 →](https://www.tradingview.com/chart/?symbol=BINANCE:SKYAIUSDT)  |  [CoinGecko →](https://www.coingecko.com/en/coins/skyai)  |  [Binance →](https://www.binance.com/zh-CN/trade/SKYAIUSDT)

## 快照数据（2026-02-18 创建时获取）

### 价格 & 市值

| 指标 | 数值 |
|------|------|
| 价格 | $0.03684 |
| 市值 | $36.79M |
| FDV | $36.79M |
| 24h 成交量 | $2.72M |
| 24h 涨跌 | -0.93% |
| 7d 涨跌 | 4.52% |

## 核心逻辑

- **解决什么问题**：跨行业的 AI 集成，基于 MCP 协议让 AI Agent 能调用各种工具
- **竞争优势**：BNB Chain 生态 + Binance Alpha 推荐
- **代币用途**：待研究
- **代币经济模型**：待研究

## 竞品对比

| 项目 | 代币 | 市值 | 特点 | 对比 |
|------|------|------|------|------|
| SkyAI | SKYAI | $36.79M | MCP 一站式 AI 生态 | |
| | | | | |

## 风险评估

- [ ] 团队背景调查
- [ ] 代币解锁计划
- [ ] 合约是否审计
- [ ] 竞品对比完成
- [ ] 社区活跃度观察
- [ ] 大户持仓集中度

## 我的判断

- **评级**：关注 / 建仓 / 观望 / 回避
- **理由**：
- **目标价**：
- **止损价**：
- **仓位计划**：

## 操作记录

| 日期 | 操作 | 价格 | 数量 | 备注 |
|------|------|------|------|------|
| | 买入/卖出 | | | |

## 跟踪笔记

### 2026-02-18
- 首次建档
